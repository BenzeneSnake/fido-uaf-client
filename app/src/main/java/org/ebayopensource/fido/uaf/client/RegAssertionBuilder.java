...
public class RegAssertionBuilder {
    ...
    //TODO:之後優化成ByteArrayOutputStream 可以直接寫 4 bytes（32-bit）
    private byte[] getCounters() throws IOException {
        ByteArrayOutputStream byteout = new ByteArrayOutputStream();

        // 註冊時重置 counter 為 0
        Preferences.setSettingsParamLong("signCounter", 0L);
        Log.d(TAG, "Registration: Counter initialized to 0");

        // Sign Counter (32-bit, 註冊時為 0)
        byteout.write(encodeInt(0)); // 低 16 bits
        byteout.write(encodeInt(0)); // 高 16 bits
        // Timer Counter (32-bit, 通常設為 0)
        byteout.write(encodeInt(0)); // 低 16 bits
        byteout.write(encodeInt(0)); // 高 16 bits
